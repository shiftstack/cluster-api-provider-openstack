...
func (r *orcNetworkReconciler) setStatusID(ctx context.Context, orcResource *orcv1alpha1.Network, id string) error {
	applyConfig := orcapplyconfigv1alpha1.Network(orcResource.Name, orcResource.Namespace).
		WithUID(orcResource.GetUID()).
		WithStatus(orcapplyconfigv1alpha1.ImageStatus().
			WithID(id))

	return r.client.Status().Patch(ctx, orcResource, ssa.ApplyConfigPatch(applyConfig), client.ForceOwnership, ssaFieldOwner(SSAIDTxn))
}
...
func (r *orcNetworkReconciler) updateStatus(ctx context.Context, orcResource *orcv1alpha1.Network, opts ...updateStatusOpt) error {
	now := metav1.NewTime(time.Now())

	statusUpdate := createStatusUpdate(ctx, orcResource, now, opts...)

	return r.client.Status().Patch(ctx, orcResource, ssa.ApplyConfigPatch(statusUpdate), client.ForceOwnership, ssaFieldOwner(SSAStatusTxn))
}
