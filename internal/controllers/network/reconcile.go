...
func (r *orcNetworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	orcResource := &orcv1alpha1.Network{}
	err := r.client.Get(ctx, req.NamespacedName, orcResource)
	if err != nil {
	...
	if !orcResource.DeletionTimestamp.IsZero() {
		return r.reconcileDelete(ctx, orcResource)
	}

	return r.reconcileNormal(ctx, orcResource)
}

func (r *orcNetworkReconciler) getResourceClient(ctx context.Context, orcResource *orcv1alpha1.Network) (clients.NetworkClient, error) {
	log := ctrl.LoggerFrom(ctx)

	clientScope, err := r.scopeFactory.NewClientScopeFromObject(ctx, r.client, r.caCertificates, log, orc.IdentityRefProvider(orcResource))
	if err != nil {
		return nil, err
	}
...
func (r *orcNetworkReconciler) reconcileNormal(ctx context.Context, orcResource *orcv1alpha1.Network) (_ ctrl.Result, err error) {
	log := ctrl.LoggerFrom(ctx)
	log.V(3).Info("Reconciling network")

	if !controllerutil.ContainsFinalizer(orcResource, Finalizer) {
		return ctrl.Result{}, r.setFinalizer(ctx, orcResource)
	}
	...
	defer func() {
		...
		err = errors.Join(err, r.updateStatus(ctx, orcResource, statusOpts...))
	...
	resourceClient, err := r.getResourceClient(ctx, orcResource)
	if err != nil {
		return ctrl.Result{}, err
	}

	glanceImage, err := func() (*images.Image, error) {
		if orcResource.Status.ID != nil {
			log.V(4).Info("Fetching existing glance image", "ID", *orcResource.Status.ID)

			image, err := resourceClient.GetImage(*orcResource.Status.ID)
		...
		if orcResource.Spec.Import != nil {
			log.V(4).Info("Importing existing glance image")

			if orcResource.Spec.Import.ID != nil {
				image, err := resourceClient.GetImage(*orcResource.Spec.Import.ID)
				if capoerrors.IsNotFound(err) {
			...
			listOpts := listOptsFromImportFilter(orcResource.Spec.Import.Filter)
			return getGlanceImageFromList(ctx, listOpts, resourceClient)
		...
		listOpts := listOptsFromCreation(orcResource)
		return getGlanceImageFromList(ctx, listOpts, resourceClient)
	}()
	if err != nil {
		return ctrl.Result{}, err
	...
	if glanceImage == nil {
		...
		glanceImage, err = createImage(ctx, orcResource, resourceClient)
		if err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	if orcResource.Status.ID == nil {
		if err := r.setStatusID(ctx, orcResource, glanceImage.ID); err != nil {
			return ctrl.Result{}, err
		}
	}
	...
	case images.ImageStatusImporting, images.ImageStatusSaving:
		addStatus(withProgressMessage(downloadingMessage("Glance is downloading image content", orcResource)))
	...
	case images.ImageStatusQueued:
		...
		if orcResource.Spec.ManagementPolicy == orcv1alpha1.ManagementPolicyUnmanaged {
		...
		if ptr.Deref(orcResource.Status.DownloadAttempts, 0) >= maxDownloadAttempts {
		...
		canWebDownload, err := r.canWebDownload(ctx, orcResource, resourceClient)
		if err != nil {
			return ctrl.Result{}, err
		}
		...
		if canWebDownload {
			...
			err := r.webDownload(ctx, orcResource, resourceClient, glanceImage)
			if err != nil {
				return ctrl.Result{}, err
			}
			...
			return ctrl.Result{}, r.uploadImageContent(ctx, orcResource, resourceClient, glanceImage)
		}
...
func (r *orcNetworkReconciler) reconcileDelete(ctx context.Context, orcResource *orcv1alpha1.Network) (_ ctrl.Result, err error) {
	log := ctrl.LoggerFrom(ctx)
	...
	if orcResource.Spec.ManagementPolicy == orcv1alpha1.ManagementPolicyUnmanaged || orcResource.Spec.ManagedOptions.GetDeletePolicy() == orcv1alpha1.DeletePolicyDetach {
		logPolicy := []any{"managementPolicy", orcResource.Spec.ManagementPolicy}
		if orcResource.Spec.ManagementPolicy == orcv1alpha1.ManagementPolicyManaged {
			logPolicy = append(logPolicy, "deletePolicy", orcResource.Spec.ManagedOptions.GetDeletePolicy())
		}
		log.V(4).Info("Not deleting Glance image due to policy", logPolicy...)
	} else {
		imageClient, err := r.getResourceClient(ctx, orcResource)
		if err != nil {
			return ctrl.Result{}, err
		}

		var glanceImage *images.Image
		glanceImage, err = getGlanceImage(ctx, orcResource, imageClient)
	...
	// Clear the finalizer
	applyConfig := orcapplyconfigv1alpha1.Image(orcResource.Name, orcResource.Namespace).WithUID(orcResource.UID)
	return ctrl.Result{}, r.client.Patch(ctx, orcResource, ssa.ApplyConfigPatch(applyConfig), client.ForceOwnership, ssaFieldOwner(SSAFinalizerTxn))
}
...
func getGlanceImage(ctx context.Context, orcResource *orcv1alpha1.Network, imageClient clients.ImageClient) (*images.Image, error) {
	log := ctrl.LoggerFrom(ctx)
	...
	imageName := getImageName(orcResource)
